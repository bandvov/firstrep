...
module.exports = {
  ...
  getAllItems: function(req, res) {
    ...
    request.on("row", function(row) {
      if (req.url == "/") {
        ...
        self.tableRows += `<tr> 
                <td><span class="glyphicon glyphicon-pencil edit" id=${row.id}>&nbsp;</span>
                <span class="glyphicon glyphicon-remove delete" style="cursor: pointer  width:15px" 
                id="${row.id}">&nbsp;</span>${row.name}</td>    
                <td>${row.description}</td><td>${row.completed ? "yes" : "no"}</td>
                </tr>`;
      }
    });
  ...
  insertItem: function(req, res) {
    ...
    ps.prepare(
      "INSERT INTO todo (name,description,completed) VALUES (@name,@description,@completed)",
      function(err) {
        console.log(err);
...
const {id,name,description,completed} = rows.recordset[0]
console.log(id,name,description);
  ...
  updateItem: function(req, res) {
    ...
    ps.input("completed", mssql.Int);
    ps.prepare(
      "UPDATE todo SET name=@name, description=@description,completed=@completed WHERE id=@id",
      function(err) {
        if (err) {
          console.log(err);
        } else {
